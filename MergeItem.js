...
export default class MergeItem {
     ...
     * @property {string} MergeFuntion
     * @property { { x: number, y: number } } position
     * @property {Object.<string, string>} style
     */
    /**
     * @param {DragableConstructor} obj 
     */
    constructor({ symbol, id, tags, MergeFuntion, position = {}, style = {} }) {
        this.symbol = symbol + "";
        this.id = id.includes("_T") && id.includes("_R") ?
            id :
            `${id}_T${new Date().getTime()}_R${Math.floor(Math.random()*16**6).toString(16)}`;
